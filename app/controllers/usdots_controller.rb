class Us_DotsController < ApplicationController
  
  def show
    @company = Company.find(params[:company_id])
    @us_dot = Us_Dot.find(params[:id])
  end
  ...
  def create
    @company = Company.find(params[:company_id])
    @us_dot = @company.carrier_mcs.build(us_dot_params) 
    @us_dot.save
  ...
  def edit
    ...
    if @us_dot.update(
      ...
      redirect_to company_carrier_mc_path(@us_dot)
    else
      flash[:danger] = "There was a problem saving your Authority"
      redirect_to company_carrier_mc_path(@us_dot)
    end
  ...
  def update
    # @company = Company.find(params[:company_id])
    # @us_dot = Mc.find(params[:id]) 
    # if @us_dot.update(cmc_params)
    #   flash[:success] = "Your #{@us_dot.authority_type} with number #{@us_dot.number} has been updated"
    #   redirect_to @company
    # else
    #   flash[:danger] = "There was a problem saving your Authority"
    #   render :edit
